...
if __name__ == '__main__':
    ...
    dat = pd.DataFrame(df_['_SPX'])
    dat['u2'] = pd.DataFrame(df_['MOV'])
    # dat.rename(columns = {'_SPX':'y'})
    dat.rename(columns = {'_SPX':'u1'}, inplace = True)
    # dat['y'] = pd.DataFrame(df_['_SPX'])
    ...
    df[['u1_n','u2_n']] = df_n[['u1','u2']].copy()
    n = len(df)

    # sliding window
    lb = 30
    pooling = 1
    window = WindowGenerator(input_width=lb, label_width=lb, shift=0, input_columns=['u1_n','u2_n'], label_columns=['u1','u2'], all_columns=df.columns)
    td = window.make_dataset(df, shuffle=True)
    train_data = td.take(2)
    val_data = td.skip(2)

    # Training; Hint: play with num_hidden = 1 or 2, and kernel_size
    model = Autoencoder(num_timesteps=lb, num_inputs=2, num_hidden=2, kernel_size=10, pooling=pooling)
    ...
    window_orig = WindowGenerator(input_width=lb, label_width=lb, shift=0, input_columns=['u1','u2'],label_columns=['u1','u2'], all_columns=df.columns)
